...
class DataPipeline:
    ...
    @coroutine
    def update(self, methods, target):
        while True:
            df = (yield)
            ...
            for col in df.columns:
                ...
                self.historic = self.historic[col].append(new_price)
                if target == 'TI':
                    self.indicators[col]['MACD'] = self.indicators[col]['MACD'].append(ti.get_indicators(update='MACD', closes=self.historic['Adj Close'], args=self.indicator_arguments))
                    self.indicators[col]['RSI']  = self.indicators[col]['RSI'].append(ti.get_indicators(update='RSI', closes=self.historic['Adj Close'], args=self.indicator_arguments))
                    self.indicators[col]['BB']   = self.indicators[col]['BB'].append(ti.get_indicators(update='BB', closes=self.historic['Adj Close'], args=self.indicator_arguments))
                    self.indicators[col]['ATR']  = self.indicators[col]['ATR'].append(ti.get_indicators(update='ATR', closes=self.historic['Adj Close'], hi=self.historic['High'], lo=self.historic['Low'],  args=self.indicator_arguments))
                if target == 'PD':
